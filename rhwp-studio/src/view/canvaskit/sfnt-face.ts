...
export function canvasKitFontFaceData(fontData: ArrayBuffer, faceIndex: number): ArrayBuffer | null {
  ...
  if (bytes.byteLength < 4) {
    return faceIndex === 0 ? fontData.slice(0) : null;
  }
  const view = new DataView(fontData);
  if (view.getUint32(0, false) !== TTC_TAG) {
    return faceIndex === 0 ? fontData.slice(0) : null;
  }
  ...
  if (faceOffset > bytes.byteLength - 12) {
    return null;
  }
  ...
  for (let tableIndex = 0; tableIndex < tableCount; tableIndex += 1) {
    ...
    if (!Number.isSafeInteger(outputLength) || outputLength > MAX_NORMALIZED_SFNT_BYTES) {
      return null;
    }
    tables.push({
      recordOffset,
  ...
  for (let tableIndex = 0; tableIndex < tables.length; tableIndex += 1) {
    ...
    selected.set(
      bytes.subarray(table.sourceOffset, table.sourceOffset + table.length),
      table.outputOffset,
    );
