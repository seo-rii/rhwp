        ...
        if (backend.key.startsWith('canvaskit')) {
          ...
          if ((diagnostics.textReplayDiagnostics?.failures?.length ?? 0) > 0) {
            hardGateViolations.push({
              ...
              backend: backend.key,
              profile,
              code: 'runtimeTextReplayFailure',
              detail: JSON.stringify(diagnostics.textReplayDiagnostics.failures),
            });
          }
          if ((diagnostics.patternDiagnostics?.surfaceFailures ?? 0) > 0) {
            hardGateViolations.push({
              sampleId: sample.id,
              backend: backend.key,
              profile,
              code: 'runtimePatternReplayFailure',
              detail: JSON.stringify(diagnostics.patternDiagnostics),
            });
          }
...
for (const result of results) {
  ...
  summary.runtimePatternDirectImageCreations +=
    diagnostics.patternDiagnostics?.directImageCreations ?? 0;
  ...
  for (const recovery of diagnostics.imageDiagnostics?.recoveries ?? []) {
    const reason = String(recovery.reason ?? 'unknown');
    summary.runtimeImageRecoveryReasonCounts[reason] = (
      summary.runtimeImageRecoveryReasonCounts[reason] ?? 0
    ) + 1;
  }
