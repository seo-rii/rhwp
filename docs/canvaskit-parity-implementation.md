...
2. Native Skia variation corpus widening: native Skia now has checked-in
   ...
   glyph/advance/bounds smoke, explicit default-axis replay, and invalid-axis
...
- `BitmapGlyph` writer widening: add Canvas2D/SVG strict writer coverage for
  ...
  negatives in strict variant selection.
- `SvgGlyph` writer widening: enable SVG exporter output for sanitized static
  ...
  SVG-in-font direct replay rejected. SVG strict replay now rejects missing
  `viewBox` and unsafe payload flags directly in renderer selection tests;
  native Skia mirrors the same static-sanitized contract negatives.
...
- native variation-font strict replay widening: native Skia exact variable-font
  ...
  explicit default-axis replay, glyph id, advance/bounds smoke, and
