...
- Text runs are still the canonical source-backed text paint contract. Layer JSON/JS exports keep the v1-compatible `style` field and also expose `paintStyle`, a paint-visible projection that excludes layout-only text state and is intended as the schema migration path toward lower-level `GlyphRun` replay.
