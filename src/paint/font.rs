...
pub struct FontFaceResource {
    ...
    /// Explicit TTC/OTC face index. This is also part of the exact face
    /// identity, but keeping it visible makes backend diagnostics and
    /// CanvasKit/native Skia typeface construction unambiguous.
    pub face_index: u32,
...
mod tests {
    ...
    #[test]
    fn portable_blob_requires_digest_and_data_ref_by_construction() {
        let portability = FontPortability::PortableBlob {
            digest: FontDigest {
        ...
        assert!(portability.is_self_contained_replayable());
        assert_eq!(portability.kind(), FontPortabilityKind::PortableBlob);
    }
}
