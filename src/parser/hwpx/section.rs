...
fn parse_col_pr_with_children(
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Empty(ref ce)) => {
                if local_name(ce.name().as_ref()) == b"colLine" {
...
<hs:sec xmlns:hp="http://www.hancom.co.kr/hwpml/2011/paragraph"
  ...
  <hp:p paraPrIDRef="0" styleIDRef="0">
    ...
    #[test]
    fn test_parse_ctrl_column_def_preserves_col_line() {
        let xml = r##"<?xml version="1.0" encoding="UTF-8"?>
<hs:sec xmlns:hp="http://www.hancom.co.kr/hwpml/2011/paragraph"
        xmlns:hs="http://www.hancom.co.kr/hwpml/2011/section">
  <hp:p paraPrIDRef="0" styleIDRef="0">
    <hp:run charPrIDRef="0">
      <hp:ctrl>
        <hp:colPr type="NEWSPAPER" layout="LEFT" colCount="2" sameSz="1" sameGap="850">
          <hp:colLine type="DASH_DOT" width="0.4 mm" color="#112233"/>
        </hp:colPr>
      </hp:ctrl>
      <hp:t>A</hp:t>
    </hp:run>
        ...
        assert_eq!(para.text, "A");
        assert_eq!(col_def.column_count, 2);
        assert!(col_def.same_width);
        assert_eq!(col_def.spacing, 850);
        assert_eq!(col_def.separator_type, 4);
        assert_eq!(col_def.separator_width, 6);
        assert_eq!(col_def.separator_color, 0x0033_2211);
    }

    #[test]
    fn test_parse_hwpx_line_type_and_width_mapping() {
