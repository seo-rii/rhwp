...
fn canvaskit_static_svg_fragment_has_path_layer(fragment: &str) -> bool {
    ...
    while cursor < bytes.len() {
        ...
        if !canvaskit_static_svg_tag_is_supported(&name) {
            return false;
        }
        ...
        if canvaskit_static_svg_tag_is_ignored_subtree(&name) {
            if !is_self_closing {
                ignored_element_depth = 1;
            }
            cursor = tag_end + 1;
        ...
        if canvaskit_static_svg_tag_has_path_layer(&name, raw_attributes) {
            has_path_layer = true;
        }
        cursor = tag_end + 1;
    }
...
fn canvaskit_static_svg_tag_is_supported(name: &str) -> bool {
    matches!(
        name,
        "svg"
            | "g"
...
fn canvaskit_static_svg_tag_is_ignored_subtree(name: &str) -> bool {
    matches!(name, "title" | "desc" | "metadata" | "defs")
}

fn canvaskit_static_svg_tag_has_path_layer(name: &str, raw_attributes: &str) -> bool {
    match name {
